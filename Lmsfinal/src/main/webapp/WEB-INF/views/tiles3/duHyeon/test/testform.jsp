...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#examtitle").hide();
		$("input#examtitle").val($("#selexamtitle").val());
	...
	function addTestRound(){
		...
		if($("input#startDate").val() <= $("input#testDate").val()){
			alert("시작일은 최소 하루 뒤여야 합니다.");
			return;
		}
		...
		const frm = document.insertTestRound;
		frm.method = "POST";
		frm.action = "<%=ctxPath%>/addTestRound.lmsfinal";
		frm.submit();
...
<div class="container w-75" style="margin: 250px; padding:0;">
	...
	<h2 style="margin-bottom: 30px;">시험</h2>
	
	<div class="innercontainer w-100">
		<form name="insertTestRound">
	    	<div class="row">
	      	<div class="col-25">
	        	<label for="classname">과목 이름</label>
	      	</div>
	      	<div class="col-75">
	        	<input type="text" id="classname" style="width:73%" value="${requestScope.mylecture.classname}" readonly/>
	      	</div>
	    </div>
	  	...
	  	<div class="row">
	    	<div class="col-25">
	      	...
	      	<div class="col-75">
	      		<input type="text" id="examtitle" name="examtitle" style="width:36%;"/>
	    ...
	    <div class="row">
	    	<div class="col-25">
	      	...
	      	<div class="col-75">
	        	<select id="disclosure" name="disclosure" style="width:73%">
	    ...
	    <div class="row">
	    	<div class="col-25">
	      	...
	      	<div class="col-75">
	      		<input type="text" id="questioncnt" name="questioncnt" style="width:36%" value=""/>
	    ...
	    <%--
	      <div class="row">
	      <div class="col-25">
	        <label for="testclfc">문제 출제 및 수정</label>
	      </div>
	      <div class="col-75">
	        <button type="button" class="btn btn-dark">출제하기</button>
	        <button type="button" class="btn btn-dark">수정하기</button>
	      </div>
	    </div>
	    	이쪽은 뺴서 다시 만드는 걸로 해야 할 것 같음 
	    	이번학기에 만든 시험지를 쫙 보여주면서 옆에 문제넣고 수정할수있는 버튼으로 다시 문제 출제 수정으로
	    	왜냐하면 지금 하려했던거는 문제지에대한 테이블이 인서트되지않았기때문에 seq가 안나옴 그리고 문제를 만드려면 시험지seq도 필요함 그리고 10문제를 출제할건데 5문제만 만들면 안되니까 
	    	이것도 잡기위해서
	   --%>
	    ...
	    <div class="row">
	    	<div class="col-25">
	      	...
	      	<div class="col-75">
	        	<button type="button" class="btn btn-dark mt-5" id="createTestpaper" style="margin:auto; width: 36%" onclick="addTestRound()">확인</button>
	      		<button type="button" class="btn btn-light mt-5" id="createTestpaper" style="margin:auto; width: 36%" onclick="javascript:history.back()">돌아가기</button>
	      	</div>
	   	</div>
	   	<input type="hidden" id="majorid" name="majorid" value="${requestScope.mylecture.majorid}"/>
	   	<input type="hidden" id="gyowonid" name="gyowonid" value="${requestScope.mylecture.gyowonid}"/>
	   	<input type="hidden" id="subjectid" name="subjectid" value="${requestScope.mylecture.subjectid}"/>
